...
def calculate_average_growth(population_data):
    ...
    for species, data in population_data.items():
        yearly_growth = np.diff(data)  # Yearly growth
...
def calculate_yearly_growth_rate(population_data):
    growth_rate = {}
    for species, data in population_data.items():
        yearly_growth_rate = np.diff(data) / data[:-1] * 100  # Percentage change
...
if __name__ == "__main__":
    # 1. Store and Analyze Population Data
    ...
    # 2. Yearly Growth Rate Calculation
    ...
    # 3. Visualize Population Trends
    visualize_population_trends(population_data)

    # 4. Species Performance Analysis
